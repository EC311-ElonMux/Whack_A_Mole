...
The code that makes up Game Control consist of countdown.v, enable_trigger.v, mole_picker.v and score_tracker.v
	...
	mole_picker.v takes in the enable signal from enable_trigger, along with the user set difficulty and the 
	moles that have been hit. Then what this module does is it uses a set of LFSRs to randomly generate a mole 
	in one of the 8 positions, and then assigns the duration the mole stays up using the difficulty along with 
	another LFSR. Within this module is a mole_control.v which takes in these times and mole positions, and 
	regulates them so that once the duration has been reached, the mole pops back down. And this module outputs 
	an 8 bit number, which essentially details whether or not a mole is present in each of the 8 possible 
	positions. And the signal from the score tracker that tells which mole hits updates the positions of the 
